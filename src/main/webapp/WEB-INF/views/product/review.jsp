    ...
    <main>
            ...
            <tbody>
                <c:forEach items="${list}" var="item">
                    <tr>
                        ...
                        <td>${item.mi_name}</td>
                        <td><fmt:formatDate value="${item.ri_reg_dt}" pattern="yyyy-MM-dd"/></td>
                        ...
                        <td>
                            ...
                            <c:if test="${item.ri_status == 1}">
                                <button class="status_btn review_blind_cancel" data-seq="${item.ri_seq}" data-status="0">블라인드 취소</button>
                            </c:if>
        ...
        <div class="pager_area">
            <c:forEach begin="1" end="${page}" var="i">
                <a href="/product/review?keyword=${keyword}&offset=${(i-1)*10}&align_type=${align_type}&review_status=${review_status}&search_type=${search_type}" class="pager">${i}</a>
            </c:forEach>
        </div>
    </main>
</body>
